...
public class ContextLoader {
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		// 若已经存在 ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE 对应的 WebApplicationContext 对象，则抛出 IllegalStatueException
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		// 根据 web.xml 中的 Init-Parameter 参数加载
		...
		// 通过 ContextLoader.properties 文件指定，默认未 XmlWebApplicationContext
		else {
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		if (ObjectUtils.identityToString(wac).equals(wac.getId())) {
			// 若 wac 使用了默认编号（类名@内存地址），则重新设置 ID 属性
