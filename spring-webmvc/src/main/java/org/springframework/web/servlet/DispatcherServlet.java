...
public class DispatcherServlet extends FrameworkServlet {
	...
	// Context-Type 为 multipart/* 的请求的解析器
	@Nullable
	private MultipartResolver multipartResolver;

	/** LocaleResolver used by this servlet. */
	// 本地化（国际化）解析器 Accept-Language
	@Nullable
	private LocaleResolver localeResolver;

	/** ThemeResolver used by this servlet. */
	// 从请求中解析出使用的主题,例如：从请求头 User-Agent, 判断使用 PC 端,还是移动端的主题
	@Nullable
	private ThemeResolver themeResolver;

	/** List of HandlerMappings used by this servlet. */
	// 处理器匹配接口，根据请求 handler 获得其他的处理器和拦截器数组
	@Nullable
	...
	// 处理器异常解析接口, 将处理器执行时发生的异常，解析成对应的 ModelAndView 结果
	@Nullable
	...
	// FlashMap 管理器接口，负责重定向，保存参数到临时存储中
	@Nullable
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		if (this.detectAllHandlerMappings) {
			...
			// 如果关闭探测功能，则获得 HANDLER_MAPPING_BEAN_NAME 对应的 Bean 对象，并设置为 handlerMapping
			try {
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// 真正调用 handler 方法,并返回 ModelAndView
				...
				// 后置处理 拦截器 触发 HandlerInterceptor.postHandler 方法
	...
	private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
		...
		// 如果是否异常的结果
		if (exception != null) {
			// 情况1:从 ModelAndViewDefinningException 中获得 ModelAndView 对象
			if (exception instanceof ModelAndViewDefiningException) {
				logger.debug("ModelAndViewDefiningException encountered", exception);
				mv = ((ModelAndViewDefiningException) exception).getModelAndView();
			}
			else {
				// 情况2:处理异常，生成 ModelAndView 对象
	...
	protected void render(ModelAndView mv, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		else { // case 2: 直接使用 ModelAndView 中的 View
