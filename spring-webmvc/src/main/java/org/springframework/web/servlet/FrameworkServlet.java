...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected WebApplicationContext initWebApplicationContext() {
		// 获取 RootWebApplicationContext 对象
		WebApplicationContext rootContext =
				WebApplicationContextUtils.getWebApplicationContext(getServletContext());
		WebApplicationContext wac = null;

		// 第一种情况，如果构造方法已经传入 WebApplicationContext 对象，直接使用
		...
		// 第三种情况：创建一个 WebApplicationContext
		if (wac == null) {
	...
	protected void doOptions(HttpServletRequest request, HttpServletResponse response)
		...
		if (this.dispatchOptionsRequest || CorsUtils.isPreFlightRequest(request)) {
			processRequest(request, response);
			// 如果响应 Header 保存 Allow 则不需要交给 super 方法
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// 从 ThreadLocal 获取 LocaleContext
		// previous 过早的
		LocaleContext previousLocaleContext = LocaleContextHolder.getLocaleContext();
		LocaleContext localeContext = buildLocaleContext(request);

		// 从 ThreadLocal 获取 RequestAttributes
		...
		try {
			...
			// 向 ApplicationContext 发布 RequestHeandleEvent
